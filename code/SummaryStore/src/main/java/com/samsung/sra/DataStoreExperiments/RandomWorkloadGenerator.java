...
public class RandomWorkloadGenerator implements WorkloadGenerator<Long> {
    ...
    @Override
    public Workload generate(long T) {
        Random random = new Random(0);
        Workload ret = new Workload();
        List<AgeLengthClass> alClasses = LogarithmicAgeLengths.getAgeLengthClasses(T, T, A, L);
        for (AgeLengthClass alClass : alClasses) {
            List<Workload.Query> thisClassQueries = new ArrayList<>();
            for (int q = 0; q < Q; ++q) {
                Pair<Long, Long> al = alClass.sample(random);
                long age = al.getFirst(), length = al.getSecond();
                long l = T - length + 1 - age, r = T - age;
                if (0 <= l && r < T) {
                    Workload.Query query = new Workload.Query(operatorType, l, r, operatorIndex, null, 0L);
